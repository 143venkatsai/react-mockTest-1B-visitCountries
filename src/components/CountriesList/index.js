...
class CountriesList extends Component {
  ...
  visitCountry = id => {
    ...
    const visitedCountriesList = activeCountriesList.map(eachCountry => {
    ...
    this.setState({activeCountriesList: visitedCountriesList})
  }

  removeCountry = id => {
    const {activeCountriesList} = this.state
    const filteredCountries = activeCountriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        const filterCountry = {
          ...eachCountry,
          isVisited: !eachCountry.isVisited,
    ...
    this.setState({activeCountriesList: filteredCountries})
  }

  render() {
    const {activeCountriesList} = this.state
    const visitedCountries = activeCountriesList.filter(
    ...
    return (
      <CountriesContainer>
        ...
        <CountriesListContainer>
          {activeCountriesList.map(eachCountry => (
            <CountryItem
